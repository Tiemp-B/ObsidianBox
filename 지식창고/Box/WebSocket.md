...
"WebSocket은 HTTP처럼 클라이언트가 요청을 보내야만 서버가 응답할 수 있는 단방향 요청-응답 방식이다"라는 서술은 오답이다 — WebSocket의 핵심은 오히려 **연결을 유지한 채 서버도 먼저 데이터를 보낼 수 있는 양방향 통신**이라는 점이며, 이것이 일반 HTTP 요청-응답과의 결정적 차이다.
