...
"Rate Monotonic Scheduling은 작업의 남은 마감시한에 따라 우선순위가 실행 중 계속 재계산되는 동적 스케줄링이다"라는 서술은 오답이다 — RMS의 핵심은 오히려 **주기에 따라 미리 고정된 우선순위를 실행 내내 그대로 유지**하는 정적 스케줄링이며, 우선순위가 실행 중 재계산되는 것은 [[Earliest Deadline First(EDF)]]의 특징이다.
