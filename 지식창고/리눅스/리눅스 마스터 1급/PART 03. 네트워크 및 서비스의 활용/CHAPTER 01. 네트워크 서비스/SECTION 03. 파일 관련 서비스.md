...
- vsftpd (Very Secure FTP Daemon)는 기본 제공하는 FTP 프로그램

### FTP 모드
명령 전달을 위한 제어포트와 데이터 전달을 위한 전송 포트를 각각 사용한다
제어 포트 : 21포트
- FTP 능동 모드
	1. Client.2220 -> "PORT 2221(수신포트)" -> Server.21
	2. Server.21 -> "OK" -> Client.2220
	3. Server.20 -> DATA CHANNEL -> Client.2221
	4. Client.2221 -> "OK" -> Server.20
- FTP 수동 모드
	1. Client.2220 -> "PASV" -> Server.21
	2. Server.21 -> "OK 3333(포트 지정. 보통 1024 이후의 포트)" -> Client.2220
	3. Client.2221 -> DATA CHANNER -> Server.3333
...
- /etc/vsftpd/user_list : vsftpd.conf의 설정에 따라 다른 동작
	...
	- `userlist_deny=YES` => 파일에 등록된 계정 접근 불가
	- `userlist_deny=NO` => user_list
