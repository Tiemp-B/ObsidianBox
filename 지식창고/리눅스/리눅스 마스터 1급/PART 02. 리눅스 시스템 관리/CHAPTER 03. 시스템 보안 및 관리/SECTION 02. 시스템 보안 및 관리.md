...
6. `password	required	pam_cracklib.so	retry=3`
	패스워드 만료시 pam_cracklib.so는 새로운 패스워드 입력 요구.
...
### sudoer 파일 편집
형식 : `[사용자명]/[%그룹명] [로그인기기명]=([계정명]:[그룹명]) [명령어]`

사용자는 ()안의 계정/그룹의 권한을 대행하여 sudo 명령어가 가능하다.
이 기능은 해당 사용자가 괄호 안의 계정/그룹의 권한을 넘어가는 동작을 막기 위해 존재

# 4. 주요 보안도구
## nmap(Network Mapper)
[[nmap]]은 네트워크 탐지 및 보안 감사를 위한 오픈 소스 도구. 단일 호스트에 대한 스캔 뿐 아니라 거대한 네트워크도 스캔 가능하도록 설계
주로 호스트가 네트워크상에서 검색 가능한지 혹은 사용하고 있는 포트가 무엇인지 스캔용도로 사용
...
목적 : 네트워크 응용 프로그램이나 데몬의 오류, 네트워크의 상태를 분석 및 원인 규명

## tripwire
[[tripwire]]는 외부 침입으로 인한 주요 파일의 변경을 탐지하는 무결성 도구이다
2000년도 트립와이어 사가 소스코드를 기부하였고, GNU GPL 2.0 라이선스를 따른다

원리 : 중요 파일들의 스냅샷을 보관하고 변경 발생 시 정상적인 수정임을 확인하여 침입 여부 확인

### Nessus
Nessus는 사용 취약점 스캐너이며 가장 많이 사용된다
Nessus Essentials는 교육용 목적으로 무료 제공되며 유료 제공으로 더 많은 기능을 제공한다

특징
- 원격 접속으로 민감한 데이터에 접근하는 취약점 탐지
- 오픈 메일 릴레이나 중요 패치 미적용 등의 잘못된 **시스템 설정 탐지**
...
- 공개키 기반 암호화
	- 비대칭키, [[RSA]]라고도 한다
	- 공개키로 암호화 한 항목은 개인키로만 복호 가능하다
- 디지털 서명
	- 개인키로 암호화 한 항목은 공개키로 복호화 할 수 있기에 서명했음을 보장한다
### John The Ripper
무료 패스워드 크래킹 도구

동작 원리
- 미리 준비한 단어 사전을 기반으로 무작위 대입 공격을 수행
