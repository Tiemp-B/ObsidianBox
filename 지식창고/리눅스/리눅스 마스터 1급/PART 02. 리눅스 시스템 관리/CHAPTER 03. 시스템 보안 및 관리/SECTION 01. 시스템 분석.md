...
| lastlog   |          | 각 사용자의 마지막 로그인 기록을 보관<br>바이너리 형식으로 lastlog 명령어로 확인 가능 |
| wtmp      |          | 각 사용자의 매 로그인과 로그아웃 기록<br>바이너리 형식의 last 명령어로 확인 가능     |
| btmp      |          | 모든 로그인 실패기록<br>바이너리 형식으로 lastb 명령어로 확인 가능             |
| utmp      |          | 사용자의 현재 로그인 상태를 보관                                    |
